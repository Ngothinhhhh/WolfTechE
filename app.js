...
app.post('/api/user/create' , async(req, res) => {
  try {
    var { user_email , user_name , user_password ,user_avt_img,  user_phone, user_gender , user_birth, user_address  } = req.body 
    // regular expression  : validate data from response
    if(user_email.trim() == '' || user_name.trim() == '' || user_phone == '' || user_password.trim() == '' || user_birth.trim() == '' || user_avt_img.trim() == '' || user_gender.trim() == '' || user_address.trim() == ''){
      return res.send(response(504,'',"Không được để trống các Ô !"));
    }

    var check = validator.isEmail(user_email) // true  or false 
    if (check == 'false'){
    ...
    req.body['user_avt_img']  = user_avt_img
